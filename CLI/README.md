...
* `String Parser` -- разбивает введенную строку на список команд 
...
* `Command Factory` -- выдает список команд, построенный на основе входа 
-->
...
В методе `start` выполняется бесконечный цикл. На каждой итерации с помощью `Reader` считывается введенная пользователем строка. Далее эта строка передается в `String Parser`, которой выдает список команд.
...
Методы:
* `substitute(String input, Map<String, String> memory): String` -- подставляет вместо переменных их значения из `memory`. Если переменная отсутствует в памяти, то подставляется пустая строка.

Метод `substitute` с помощью конечного автомата выделяет внешние кавычки и вместо переменных вне одинарных кавычек подставляет их значения
...
3. Текст внутри `"` (включительно) является единым токеном, игнорируется сочетание `\"` и предыдущие правила. Это правило работает, если `"` не находится между `'`
...
Методы:
* `execute(List<String> args)` -- выполнение команды с использованием аргументов `args`
...
`execute(List<String> args)` ожидает 1 аргумент, если он не был передан ранее, в противном случае 0
...
`execute(List<String> args)` произвольное количество аргументов
...
`execute(List<String> args)` игнорирует аргументы
...
`execute(List<String> args)` игнорирует аргументы. Отсюда можно кинуть исключение о завершении (зависит от выбранного языка, в Java так например делать не принято)
...
`execute(List<String> args)` аргументы добавляются к тем, что были переданные в конструкторе, зовется соответсвующая команда

`stdout`, `stderr`, `returnCode` хранят соответствующие значения 

## **Command Factory**
Класс, отвечающий за преобразование пред обработанных групп токенов в экземпляры класса `Command`

Методы:
* `tokensToCommands(List<String> tokens)` -- для каждой группы токенов понимает присваивание ли это или нет (второй токен `=`), если да, то создает команду присваивания с соответствующими аргументами, иначе смотрим на первый токен. Если первый токен это кавычки, то кидаем ошибку. Иначе воспринимаем первый токен как команду, а все последующие токены ее аргументами, конструируем соответсвующий `Command` (если это ни одна из предопределенных команд, то считаем ее `OtherCommand`). Выдает список команд
...
Методы:
* `execute(List<Command> commands)` -- идет по каждой команде и пытается выполнить ее. Если команда завершается с ошибкой, то останавливает выполнение и возвращает stdout и stderr последней команды. При успешном выполнении команды берет ее stdout и подает в качестве аргумента следующей команде, либо выдает наружу. Если выполненная команда оказалось командой `exit`, то обновляет `isTerminated`.
